...
def SolveOnCellKeopsGrid(muX,subMuY,subY,posX,posY,rhoX,rhoY,alphaInit,eps,SinkhornError=1E-4,SinkhornErrorRel=False,YThresh=1E-14,autoEpsFix=True,verbose=True,SinkhornMaxIter = None,boxDim = None):
    ...
    KeposX = KeposX.view(2, 2, cellsize, cellsize, 2).permute((0,2,1,3,4)).reshape(-1,dim) # here a view is not possible in conjuction with the permute
    ...
    # Get transport plan
    P = torch.exp((alpha.reshape(-1,1) + beta.view(1,-1) - 0.5*torch.sum((KeposX.reshape(-1, 1, dim) - KesubPosY.view(1, -1, dim))**2, axis = 2))/blur**2)*KemuX.reshape(-1,1)*KesubRhoY.view(1,-1)
