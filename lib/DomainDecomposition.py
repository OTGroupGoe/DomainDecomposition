...
import scipy
import torch
import geomloss@softmin-grid as geomloss
...
def SolveOnCellKeopsGrid(muX,muY,subMuY,subY,posX,posY,rhoX,rhoY,alphaInit,eps,SinkhornError=1E-4,SinkhornErrorRel=False,YThresh=1E-14,autoEpsFix=True,verbose=True,SinkhornMaxIter = None,boxDim = [0,0]):
    ...
    alpha,beta = sinkhorn_divergence_two_grids(
        a,
        b,
        p=2,
        blur=blur,
...
#TODO remove this, this was just a temporary work around
    ...
    **kwargs,
    ...
    Args:
        ...
        potentials (bool, optional): Should we return the optimal dual potentials
            ...
            Defaults to False.
        dx (float, optional): spacing between grid points
    Returns:
        (B,) Tensor or pair of (B, Mx, ...), (B, Nx, ...) Tensors: If `potentials` is True,
            we return a pair of (B, Mx, ...), (B, Nx, ...) Tensors that encode the optimal
    ...
    if cost is None:
        # Dummy cost matrices
        C_xxs = [(p, dx, a_i.shape[2:]) for a_i in a_logs]
        C_yys = [(p, dx, b_i.shape[2:]) for b_i in b_logs]
        C_xys = [(p, dx, a_i.shape[2:]) for a_i in a_logs]
        C_yxs = [(p, dx, b_i.shape[2:]) for b_i in b_logs]
    else:
