...
def batch_cell_marginals_2D(marg_indices, marg_data, shapeY):
    ...
    for (i, indices) in enumerate(marg_indices):
        left[i], bottom[i], width[i], height[i], idx_i, idy_i = reformat_indices_2D(indices, shapeY)
        idx.append(idx_i)
        idy.append(idy_i)
...
def batch_balance(muXCell, Nu_basic):
    B, M, _ = muXCell.shape
    s = M//2
    atomic_mass = muXCell.view(B, 2, s, 2, s).permute((0, 1, 3, 2, 4)).contiguous().sum(dim = (3, 4))
    atomic_mass = atomic_mass.view(B, -1).cpu().numpy()
    for k in range(B):
        status, Nu_basic[k] = DomDec.BalanceMeasuresMulti(Nu_basic[k], atomic_mass[k], 1e-12, 1e-7)
    return Nu_basic
...
class LogSinkhornCudaImageOffset(LogSinkhornGPU.AbstractSinkhorn):
    ...
    def __init__(self, mu, nu, C, eps, **kwargs):
        ...
        self.offsetX, self.offsetY, self.offset_const = compute_offsets_sinkhorn_grid(xs, ys, eps)
    ...
    def get_new_alpha(self):
        ...
        return - self.eps * (LogSinkhornGPU.softmin_cuda_image(h, Ms, Ns, self.eps, dx) 
    ...
    def get_new_beta(self):
        ...
        return - self.eps * (LogSinkhornGPU.softmin_cuda_image(h, Ns, Ms, self.eps, dx) 
    ...
    def get_dense_plan(self, ind = None, C = None):
        ...
        pi = torch.exp((alpha.view(B,-1,1) + beta.view(B,1,-1) - C)/self.eps) * muref.view(B,-1,1) * nuref.view(B,1,-1)
        return pi
...
def get_cell_marginals(muref, nuref, alpha, beta, xs, ys, eps):
    ...
    alpha_b = alpha.view(-1, 2, s, 2, s).permute((0,1,3,2,4)).reshape(-1, s, s)
    mu_b = muref.view(-1, 2, s, 2, s).permute((0,1,3,2,4)).reshape(-1, s, s)
    new_Ms = (s, s)
    ...
    offsetX, offsetY, offset_const = compute_offsets_sinkhorn_grid(xs_b, ys_b, eps)
    h = alpha_b / eps + logmu_b + offsetX
    beta_hat =  - eps * (LogSinkhornGPU.softmin_cuda_image(h, Ns, new_Ms, eps, dx) 
            + offsetY + offset_const)
    
    # Build cell marginals
    nu_basic = nuref[:,None] * torch.exp((beta[:,None] - beta_hat.view(-1, n_basic, *Ns))/eps)
    return nu_basic

def BatchIterate(\
        muY,posY,dx,eps,\
        partitionDataCompCells,partitionDataCompCellIndices, # TODO: remove this unnecesary input
    ...
    for i in range(0, BatchTotal, BatchSize):
        ...
        posXBatch = tuple(x[i:i+BatchSize] for x in posXJ) # posXJ is list of coordinates along dimensions
        ...
        lenJ = len(partitionDataCompCellsBatch[0]) # All composite cells have same size
...
def BatchDomDecIteration_CUDA(\
    ...
    muYCell, left, bottom, width, height = batch_cell_marginals_2D(muYCellIndices, muYCellData, shapeY)
    ...
    # 5. Solve problem
    msg,resultAlpha,resultBeta,Nu_basic,solver=BatchSolveOnCell_CUDA(muXCell,muYCell,posXCell,posYCell,eps,alphaCell,subMuY,SinkhornError,SinkhornErrorRel, SinkhornMaxIter = SinkhornMaxIter, SinkhornInnerIter =SinkhornInnerIter)
    ...
    MuYAtomicIndicesList, MuYAtomicDataList = unpack_cell_marginals_2D(Nu_basic, left, bottom, shapeY)
